...
class MoveableWidget(gui.EventSource, MixinPositionSize):
    ...
    def on_drag(self, emitter, x, y):
        if self.active:
            self.set_position(
                float(x) - float(self.attr_width) / 2.0,
                float(y) - float(self.attr_height) / 2.0,
            )
        return (x, y)
...
class LinkView(gui.SvgPolyline, FBD_model.Link):
    ...
    def update_path(self, emitter=None):
        ...
        xsource += w
        ysource += h / 2
        xsource_parent, ysource_parent = self.get_absolute_node_position(
            self.source.get_parent()
        )
        ...
        xdestination_parent, ydestination_parent = self.get_absolute_node_position(
            self.destination.get_parent()
        )
        (
            wdestination_parent,
            hdestination_parent,
        ) = self.destination.get_parent().get_size()
        ...
        if xdestination - xsource < offset * 2:
            ...
            if ydestination > ysource:
                ...
                self.add_coord(
                    xsource + offset,
                    (ysource_parent + hsource_parent)
                    + (ydestination_parent - (ysource_parent + hsource_parent)) / 2.0,
                )
                self.add_coord(
                    xdestination - offset,
                    (ysource_parent + hsource_parent)
                ...
                self.add_coord(
                    xsource + offset,
                    (ydestination_parent + hdestination_parent)
                    + (ysource_parent - (ydestination_parent + hdestination_parent))
                    / 2.0,
                )
                self.add_coord(
                    xdestination - offset,
        ...
        if self.bt_unlink is not None:
            w, h = self.bt_unlink.get_size()
            self.bt_unlink.set_position(
                xdestination - offset / 2.0 - w / 2, ydestination - h / 2
            )
...
class ObjectBlockView(FBD_model.ObjectBlock, gui.SvgSubcontainer, MoveableWidget):
    ...
    def __init__(self, obj, container, x=10, y=10, *args, **kwargs):
        ...
        gui.SvgSubcontainer.__init__(
            self, x, y, self.calc_width(), self.calc_height(), *args, **kwargs
        )
    ...
    def add_io_widget(self, widget):
        ...
        self.append(widget)
        widget.onmousedown.do(
            self.container.onselection_start,
            js_stop_propagation=True,
            js_prevent_default=True,
        )
        widget.onmouseup.do(
            self.container.onselection_end,
            js_stop_propagation=True,
            js_prevent_default=True,
        )
...
class TextInputAdapter(ObjectBlockView):
    def __init__(self, obj, container, x=10, y=10, *args, **kwargs):
        ...
        txt = gui.TextInput()
        ofbv = ObjectFunctionBlockView(
            self.reference_object, txt.get_value, "get_value", "get_value", self
        )
        ofbv.add_io_widget(OutputView("Value"))
        self.add_fb_view(ofbv)

        ofbv = ObjectFunctionBlockView(
            self.reference_object, txt.set_value, "set_value", "set_value", self
        )
        self.add_fb_view(ofbv)
...
class FunctionBlockView(FBD_model.FunctionBlock, gui.SvgSubcontainer, MoveableWidget):
    ...
    def __init__(self, name, container, x=10, y=10, *args, **kwargs):
        ...
        self.input_event.set_size(
            len(self.input_event.name) * self.io_font_size, self.io_font_size
        )
        self.input_event.onmousedown.do(
            self.container.onselection_start,
            js_stop_propagation=True,
            js_prevent_default=True,
        )
        self.input_event.onmouseup.do(
            self.container.onselection_end,
            js_stop_propagation=True,
            js_prevent_default=True,
        )
    ...
    def populate_io(self):
        ...
        for arg in signature.parameters:
            self.add_io_widget(
                InputView(arg, default=signature.parameters[arg].default)
            )

    def calc_height(self):
        inputs_count = 0 if self.inputs is None else len(self.inputs)
        outputs_count = 0 if self.outputs is None else len(self.outputs)
        return (
            self.label_font_size
            + (max(outputs_count, inputs_count) + 2) * self.io_font_size
        )
    ...
    def calc_width(self):
        ...
        return (
            max(
                (len(self.name) * self.label_font_size),
                (max(max_name_len_input, max_name_len_output) * self.io_font_size) * 2,
            )
            + self.io_left_right_offset
        )
...
class ObjectFunctionBlockView(FunctionBlockView):
    ...
    def __init__(
        self, obj, method, method_name, name, container, x=10, y=10, *args, **kwargs
    ):
        self.reference_object = obj
        self.method = method
    ...
    def do(self, *args, **kwargs):
        if kwargs.get("EN") is not None:
            if kwargs["EN"] is False:
                return
        if "EN" in kwargs:
            del kwargs["EN"]
...
class ProcessView(gui.Svg, FBD_model.Process):
    ...
    def onselection_end(self, emitter, x, y):
        ...
        if self.selected_input is not None and self.selected_output is not None:
...
class FBToolbox(gui.Container):
    ...
    def add_widget_to_collection(
        self, functionBlockClass, group="standard_tools", **kwargs_to_widget
    ):
...
class FBHelper(gui.HBox):
    ...
    def __init__(self, appInstance, functionBlockClass, **kwargs_to_widget):
        ...
        if hasattr(functionBlockClass, "icon"):
            ...
            elif functionBlockClass.icon is None:
            ...
            icon_file = (
                "/editor_resources:widget_%s.png" % self.functionBlockClass.__name__
            )
    ...
    def build_widget_name_list_from_tree(self, node):
        if not issubclass(type(node), FBD_model.FunctionBlock) and not issubclass(
            type(node), ProcessView
        ):
            return
    ...
    def create_instance(self, widget):
        ...
        function_block = self.functionBlockClass(
            variableName, self.appInstance.process, **self.kwargs_to_widget
        )
...
class MyApp(App):
    ...
    def main(self):
        self.main_container = gui.AsciiContainer(
        ...
        self.process.onfunction_block_clicked.do(
            self.onprocessview_function_block_clicked
        )
