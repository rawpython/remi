...
class STRING(FBD_view.FunctionBlockView):
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific", """Defines the actual value""", str, {}
    )
    def value(self):
...
class BOOL(FBD_view.FunctionBlockView):
    @property
    @gui.editor_attribute_decorator(
        "WidgetSpecific", """Defines the actual value""", bool, {}
    )
    def value(self):
        if len(self.outputs) < 1:
            return False
...
class PULSAR(FBD_view.FunctionBlockView):
    ...
    def do(self):
        OUT = (
            int((time.time() - self.tstart) * 1000) % (self.ton + self.toff)
        ) < self.ton
        return OUT
