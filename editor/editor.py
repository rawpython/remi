...
class DraggableItem(gui.EventSource):
    ...
    def start_drag(self, emitter, x, y):
        self.active = True
        self.app_instance.mainContainer.onmousemove.do(
            self.on_drag, js_prevent_default=True, js_stop_propagation=False
        )
        self.app_instance.mainContainer.onmouseup.do(
            self.stop_drag, js_prevent_default=True, js_stop_propagation=False
        )
        self.app_instance.mainContainer.onmouseleave.do(
            self.stop_drag, 0, 0, js_prevent_default=True, js_stop_propagation=False
        )
        self.origin_x = -1
        self.origin_y = -1
    ...
    def stop_drag(self, emitter, x, y):
        ...
        self.update_position()
        self.app_instance.mainContainer.onmousemove.do(
            None, js_prevent_default=True, js_stop_propagation=True
        )
        self.app_instance.mainContainer.onmouseup.do(
            None, js_prevent_default=True, js_stop_propagation=True
        )
        self.app_instance.mainContainer.onmouseleave.do(
            None, 0, 0, js_prevent_default=True, js_stop_propagation=True
        )
        return ()
...
class SvgDraggablePoint(gui.SvgRectangle, DraggableItem):
    ...
    def setup(self, refWidget, newParent):
        if type(refWidget) in self.compatibility_iterable or refWidget is None:
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(
                float(self.refWidget.attributes[self.name_coord_x]) - self.w / 2,
                float(self.refWidget.attributes[self.name_coord_y]) - self.h / 2,
            )
...
class SvgDraggableRectangleResizePoint(gui.SvgRectangle, DraggableItem):
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(
                float(self.refWidget.attributes["x"])
                + float(self.refWidget.attributes["width"])
                - self.w / 2,
                float(self.refWidget.attributes["y"])
                + float(self.refWidget.attributes["height"])
                - self.h / 2,
            )
...
class SvgDraggableCircleResizeRadius(gui.SvgRectangle, DraggableItem):
    ...
    def update_position(self):
        if self.refWidget:
            self.set_position(
                float(self.refWidget.attributes["cx"])
                + float(self.refWidget.attributes["r"])
                - self.w / 2,
                float(self.refWidget.attributes["cy"]) - self.h / 2,
            )
...
class ResizeHelper(gui.Widget, DraggableItem):
    ...
    def setup(self, refWidget, newParent):
        ...
        if issubclass(newParent.__class__, gui.TabBox):
            return
        if (
            issubclass(refWidget.__class__, gui.Widget)
            and "left" in refWidget.style
            and "top" in refWidget.style
            and "width" in refWidget.style
            and "height" in refWidget.style
            and refWidget.css_position == "absolute"
        ):
            DraggableItem.setup(self, refWidget, newParent)
        ...
        if self.active:
            if self.origin_x == -1:
                ...
                self.refWidget.style["width"] = gui.to_pix(
                    self.round_grid(self.refWidget_origin_w + float(x) - self.origin_x)
                )
                self.refWidget.style["height"] = gui.to_pix(
                    self.round_grid(self.refWidget_origin_h + float(y) - self.origin_y)
                )
                self.update_position()
        ...
        if self.active:
            if self.origin_x == -1:
                ...
                self.refWidget.style["left"] = gui.to_pix(
                    self.round_grid(self.refWidget_origin_x + float(x) - self.origin_x)
                )
                self.refWidget.style["top"] = gui.to_pix(
                    self.round_grid(self.refWidget_origin_y + float(y) - self.origin_y)
                )
                self.update_position()
...
class Project(gui.Container):
    ...
    def load(self, ifile, configuration):
        ...
        if app_init_fnc is None:
            return None

        members_list = app_init_fnc.__dict__.values()
        for m in members_list:
            if inspect.isfunction(m) and m.__name__ not in [
                "__init__",
                "main",
                "idle",
                "construct_ui",
    ...
    def check_pending_listeners(self, widget, widgetVarName, force=False):
        ...
        for event in self.pending_listener_registration:
            if force or (
                hasattr(event["eventsource"], "path_to_this_widget")
                and hasattr(event["eventlistener"], "path_to_this_widget")
            ):
                if (
                    force
                    or (
                        widget.variable_name in event["eventsource"].path_to_this_widget
                        and widget.variable_name
                        in event["eventlistener"].path_to_this_widget
                    )
                ) and event["done"] is False:
                    ...
                    if (
                        len(source_filtered_path) == 0
                        and event["eventsource"].attr_editor_newclass is False
                    ):
                        sourcename = event["eventsource"].variable_name

                    #    listenername = event['eventlistener'].identifier
                    if force or (
                        self.children["root"] == widget
                        and widget.attr_editor_newclass is not True
                    ):
                    ...
                    if len(source_filtered_path) > 0:
                        sourcename = (
                            "%s.children['"
                            + "'].children['".join(source_filtered_path)
                            + "']"
                        ) % sourcename
                    ...
                    if (
                        len(listener_filtered_path) == 0
                        and event["eventlistener"].attr_editor_newclass is False
                    ):
                        listenername = event["eventlistener"].variable_name
                    if len(listener_filtered_path) > 0:
                        listenername = (
                            "%s.children['"
                            + "'].children['".join(listener_filtered_path)
                            + "']"
                        ) % listenername

                    code_nested_listener += prototypes.proto_set_listener % {
                    ...
                    if (
                        event["eventlistener"].identifier
                        not in self.code_declared_classes
                    ):
                        self.code_declared_classes[
                            event["eventlistener"].identifier
                        ] = ""

                    if event["eventlistener"].attr_editor_newclass is True:
                        if not event["skip_function_definition"]:
                            self.code_declared_classes[
                                event["eventlistener"].identifier
                            ] += event["listenerClassFunction"]
    ...
    def repr_widget_for_editor(self, widget, first_node=False):
        ...
        for x, y in inspect.getmembers(widget.__class__):
            if type(y) == property and getattr(widget, x) is not None:
                if hasattr(
                    y.fget, "editor_attributes"
                ):  # if this property is visible for the editor
                    _value = getattr(widget, x)
                    if type(_value) == type("") or type(_value) == type(""):
                        _value = '"%s"' % _value
        ...
        for setOnEventListenerFuncname, setOnEventListenerFunc in inspect.getmembers(
            ...
            if issubclass(type(setOnEventListenerFunc), gui.ClassEventConnector):
                ...
                if setOnEventListenerFunc.callback is not None and hasattr(
                    setOnEventListenerFunc.event_method_bound, "_event_info"
                ):
                    ...
                    if (
                        hasattr(listener, listenerFunctionName)
                        and listenerFunction.__code__
                        != editor_widgets.fakeListenerFunc.__code__
                    ):
                    ...
                    self.pending_listener_registration.append(
                        {
                            "done": False,
                            "eventsource": widget,
        ...
        for child_key in widget.children.keys():
            ...
            children_code_nested += prototypes.proto_layout_append % {
                "parentname": widgetVarName,
                "varname": "%s,'%s'" % (child.variable_name, child.variable_name),
            }
        ...
        if widget.attr_editor_newclass:
            if widget.identifier not in self.code_declared_classes:
                self.code_declared_classes[widget.identifier] = ""

            self.code_declared_classes[widget.identifier] = (
                prototypes.proto_code_class
                % {
                    "classname": classname,
                    "superclassname": widget.attr_class,
    ...
    def export_widget_for_app_template(self, widget, first_node=False):
            ...
            if type(y) == property and getattr(widget, x) is not None:
                if hasattr(
                    ...
                    if type(_value) == str:
                        _value = '"%s"' % _value
                    code_nested += prototypes.proto_property_setup % {
                        "varname": "self" if first_node else widgetVarName,
                        "property": x,
                        "value": _value,
                    }
        else:
        ...
        if widget.attr_editor_newclass or first_node:
            ...
            if first_node:
                ...
                self.code_declared_classes[widget.identifier] = (
                    prototypes.proto_export_app_template
                    % {
                        "classname": classname,
                        "superclassname": widget.attr_class,
                        "nested_code": code_nested + children_code_nested,
    ...
    def save(self, save_path_filename, configuration):
        ...
        main_code_class = prototypes.proto_code_main_class % {
            "classname": configuration.configDict[configuration.KEY_PRJ_NAME],
            "config_resourcepath": configuration.configDict[
                configuration.KEY_RESOURCEPATH
            ],
        ...
        for w in self.known_project_children:
            if (
                w.__module__ not in modules_to_import
                and w.__module__ != "__main__"
                and w.__module__ != "project"
            ):
        ...
        compiled_code = prototypes.proto_code_program % {
            "imports": "\n".join(
                ["from " + modulename + " import *" for modulename in modules_to_import]
            ),
        ...
        if save_path_filename is not None:
...
class Editor(App):
    ...
    def idle(self):
        ...
        if self.projectPathFilename is not None and len(self.projectPathFilename) > 0:
    ...
    def main(self):
        ...
        m4 = gui.MenuItem(
            "Became a Sponsor", width=200, height="100%", style={"font-weight": "bold"}
        )
        ...
        grid_size = gui.HBox(
            children=[lbl, self.spin_grid_size],
            style={"outline": "1px solid gray", "margin": "2px", "margin-left": "10px"},
        )
        ...
        self.fileSaveAsDialog = editor_widgets.EditorFileSaveDialog(
            "Project Save",
            "Select the project folder and type a filename",
            False,
            ".",
            False,
            True,
        ...
        self.mainContainer.append(
            {
                "toolbox": self.widgetsCollection,
                "signals": self.signalConnectionManager,
                "menubar": menubar,
            }
        )
        ...
        self.drag_helpers = [
            ...
            SvgDraggablePoint(
                ...
                "x",
                "y",
                [gui.SvgRectangle, gui.SvgText, gui.SvgImage, gui.SvgSubcontainer],
            ),
            SvgDraggableRectangleResizePoint(
                self, [gui.SvgRectangle, gui.SvgImage, gui.SvgSubcontainer]
            ),
        ]
        for drag_helper in self.drag_helpers:
    ...
    def on_drag_resize_end(self, emitter):
        self.selectedWidget.__dict__["properties"].set_widget(self.selectedWidget)
    ...
    def configure_widget_for_editing(self, widget):
        ...
        for setOnEventListenerFuncname, setOnEventListenerFunc in inspect.getmembers(
            ...
            if hasattr(setOnEventListenerFunc, "_event_info"):
                setOnEventListenerFunc.__class__ = (
                    editor_widgets.ClassEventConnectorEditor
                )
    ...
    def on_widget_selection(self, widget):
        ...
        if issubclass(widget.__class__, gui.Container) or widget is None:
            self.mainContainer.append(self.widgetsCollection, "toolbox")
        else:
            self.mainContainer.append(
                gui.Label(
        ...
        if self.selectedWidget.__dict__.get("properties", None) is None:
            self.selectedWidget.__dict__[
                "properties"
            ] = editor_widgets.EditorAttributes(
                self, width="100%", height="100%", style={"overflow": "auto"}
            )
            self.selectedWidget.__dict__["properties"].set_widget(self.selectedWidget)
        self.mainContainer.append(
            self.selectedWidget.__dict__["properties"], "properties"
        )
    ...
    def on_open_dialog_confirm(self, widget, filelist):
        if len(filelist):
            ...
            try:
                ...
                self.show_error_dialog(
                    "ERROR: Unable to load the project",
                    "There were an error during project load: %s"
    ...
    def menu_save_clicked(self, widget, path=""):
        ...
        if len(path):
            self.projectPathFilename = (
                path + "/" + self.fileSaveAsDialog.get_fileinput_value()
            )
        else:
    ...
    def menu_export_widget_clicked(self, widget, path=""):
        ...
        if len(path):
            ...
            self.fileSaveAsDialog.confirm_value.do(self.menu_export_widget_clicked)
            self.fileSaveAsDialog.show()
            return

        code = ""
        code = code + self.project.export_widget_for_app_template(
            self.selectedWidget, True
        )
        for key in self.project.code_declared_classes.keys():
    ...
    def menu_paste_selection_clicked(self, widget):
        if self.editCuttedWidget is not None:
            key = (
                "root"
                if self.selectedWidget == self.project
                else self.editCuttedWidget.identifier
            )
    ...
    def menu_became_a_sponsor(self, widget):
        dialog = gui.GenericDialog(
            "Became a Sponsor",
            "This editor is made for you with passion. \nIt would be fantastic if you give a contribution, also a little one. ;-)",
            width="700px",
        )
        ...
        sponsor_card.attributes[
            "src"
        ] = "https://github.com/sponsors/dddomodossola/card"
    ...
    def move_widget(self, css_key, value):
        ...
        if (
            issubclass(self.selectedWidget.__class__, gui.Widget)
            and css_key in self.selectedWidget.style
        ):
