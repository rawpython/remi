...
class PLCSiemens(Image):
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific", "The IP address as string", str, {}
    )
    def ip_address(self):
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific",
        "The rack number as integer",
        int,
        {"possible_values": "", "min": -1, "max": 65535, "default": 0, "step": 1},
    )
    def rack(self):
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific",
        "The update interval in seconds as float",
        float,
        {"possible_values": "", "min": 0.0, "max": 655350.0, "default": 1.0, "step": 1},
    )
    ...
    snap7_client = (
        snap7.client.Client()
    )  # the snap7.client.Client() instance that manages data exchange with PLC
    connected = False
    ...
    def connect(self):
        try:
            if self.rack < 0 or self.slot < 0:
                return
            print(
                "connecting to ip:%s  rack:%s  slot:%s"
                % (self.ip_address, self.rack, self.slot)
            )
    ...
    def set_bool(self, db_index, byte_index, bit_index, value):
        reading = self.snap7_client.db_read(
            db_index, byte_index, 1
        )  # read 1 byte from db 31 staring from byte 120
        snap7.util.set_bool(reading, 0, bit_index, value)  # set a value of fifth bit
        self.snap7_client.db_write(
            db_index, byte_index, reading
        )  # write back the bytearray and now the boolean value is changed
...
class SiemensWidget(object):
    ...
    def do(self, callback, *userdata):
        ...
        if hasattr(self.link_to.event_method_bound, "_js_code"):
            self.link_to.event_source_instance.attributes[self.link_to.event_name] = (
                self.link_to.event_method_bound._js_code
                % {
                    "emitter_identifier": self.link_to.event_source_instance.identifier,
                    "event_name": self.link_to.event_name,
                }
            )
...
class _Mixin_Byte_property:
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific",
        "The byte index in the DB as integer",
        int,
        {"possible_values": "", "min": -1, "max": 65535, "default": 0, "step": 1},
    )
    def byte_index(self):
...
class _Mixin_Bit_property:
    ...
    @gui.editor_attribute_decorator(
        "WidgetSpecific",
        "The bit index in the byte as integer",
        int,
        {"possible_values": "", "min": -1, "max": 65535, "default": 0, "step": 1},
    )
    def bit_index(self):
...
class SiemensButton(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.button = gui.Button(
            button_label, width="100%", height="100%", style=_style
        )
        self.led = gui.Widget(
            width=15,
            height=5,
    ...
    def set_bit(self, emitter, *args, **kwargs):
        ...
        self.plc_instance.set_bool(
            self.db_index, self.byte_index, self.bit_index, value
        )
    ...
    def reset_bit(self, emitter, x, y, *args, **kwargs):
        ...
        if not self.toggle:
            self.plc_instance.set_bool(
                self.db_index, self.byte_index, self.bit_index, 0
            )
    ...
    def update(self, *args):
        ...
        if self.plc_instance is None:
            return
        if self.db_index < 0 or self.byte_index < 0 or self.bit_index < 0:
            return
        value = self.plc_instance.get_bool(
            self.db_index, self.byte_index, self.bit_index
        )
        self._set_value(value)


class BitStatusWidget(
    HBox, SiemensWidget, _Mixin_DB_property, _Mixin_Byte_property, _Mixin_Bit_property
):
