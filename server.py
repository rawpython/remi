...
class BaseApp(BaseHTTPRequestHandler, object):
    ...
    def instance(self):
        ...
        if not(k in clients.keys()):
            ...
            clients[k] = self
            clients[
                k].attachments = "<script>var sendCommand = function (url,params) { var request = new XMLHttpRequest(); request.open('POST', url, false); request.send(params); return request.responseText;};</script>"
    ...
    def processAll(self, function, paramDict, isPost):
        ...
        if ispath:
            ...
            if ret == None:
                ...
                self.wfile.write(
                    "<link href='" + BASE_ADDRESS + "style.css' rel='stylesheet' />")
