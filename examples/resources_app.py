...
class MyApp(App):
    ...
    def main(self):
        ...
        local_image = gui.Image(
            gui.load_resource("./res/mine.png"), width="30", height="30"
        )

        standard_widget = gui.Widget(
            width="30", height="30", style={"background-repeat": "no-repeat"}
        )

        standard_widget2 = gui.Widget(
            width="30", height="30", style={"background-repeat": "no-repeat"}
        )
        ...
        standard_widget.style["background-image"] = gui.to_uri(
            "/my_res_folder:mine.png"
        )
        standard_widget2.style["background-image"] = gui.to_uri(
            gui.load_resource("./res/mine.png")
        )

        print(gui.to_uri("/my_res_folder:mine.png"))
        print(gui.to_uri(gui.load_resource("./res/mine.png")))
        main_container = gui.VBox(
            children=[resource_image, local_image, standard_widget, standard_widget2],
            width=200,
            height=300,
