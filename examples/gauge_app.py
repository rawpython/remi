...
class Gauge(gui.Svg):
    def __init__(self, width, height, _min, _max):
        ...
        for i in range(0, 11):
            xy1 = self.value_to_xy_tuple(
                self.min + self.scale_value_range / 10 * i, self.radius * 92.0 / 100.0
            )
        ...
        self.arrow_preview.add_coord(
            -self.radius * 13.0 / 100.0, self.radius * 5.0 / 100.0
        )
    ...
    def value_to_angle(self, value):
        return (
            self.base_angle
            + (value - self.min) * self.scale_angle_range / self.scale_value_range
        )  # subtraction in order to go clockwise
    ...
    def angle_to_value(self, angle):
        ...
        print(
            ...
            + str(
                ...
                * self.scale_value_range
                / self.scale_angle_range
                + self.min
            )
        )
        return (
            angle - self.base_angle
        ) * self.scale_value_range / self.scale_angle_range + self.min

    def value_to_xy_tuple(self, value, radius):
        return [
            math.cos(self.value_to_angle(value)) * radius + self.radius,
            self.radius - math.sin(self.value_to_angle(value)) * radius,
        ]
    ...
    def set_value_preview(self, value):
        ...
        angle = self.value_to_angle(value)
        xy = self.value_to_xy_tuple(value, self.radius - 10)
        self.arrow_preview.attributes["transform"] = "translate(%s,%s) rotate(%s)" % (
            xy[0],
            xy[1],
            math.degrees(-angle),
        )
    ...
    def onmousedown(self, widget, x, y):
        value = self.xy_tuple_to_value(
            [float(x) - self.radius, -(float(y) - self.radius)]
        )
