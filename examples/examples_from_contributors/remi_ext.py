...
class FoldButton(gui.Button):
    ...
    def clicked(self, emitter):
        ...
        for r in self.nested_rows:
            r.style["display"] = (
                "none" if r.style["display"] == "table-row" else "table-row"
            )
...
class TreeTable(SingleRowSelectionTable):
    ...
    def append_row(self, value, key=""):
        ...
        modified_row.children[
            modified_row._render_children_list[self.fold_level + 1]
    ...
    def begin_fold(self):
        ...
        self.current_row.children["fold_cell_0"].append(
            self.current_fold_button[str(self.fold_level)], "fold"
        )
        txt = self.current_row.children["fold_cell_0"].get_text()
        self.current_row.children["fold_cell_0"].remove_child(
            self.current_row.children["fold_cell_0"].children["text"]
        )
