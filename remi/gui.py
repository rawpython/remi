...
class Gui(object):
    ...
    def __init__(self, proxy=None):
        self.proxy = proxy
        self.scene = []
        functions_ = [ 'Widget', 'Container', 'GridBox', 'HBox', 'VBox', 'Button', 'TextInput', 'SpinBox', 'Label', 'GenericDialog', 
                       'AsciiContainer', 'TabBox', 'InputDialog', 'ListView', 'ListItem','Progress','CheckBoxLabel','Link', 
                      'DropDown', 'DropDownItem', 'Image', 'Table', 'TableRow', 'TableItem', 'TableTitle', 'Input', 'Slider',
                      'ColorPicker', 'Date', 'GenericObject', 'FileFolderNavigator', 'FileFolderItem', 'FileSelectionDialog',
                      'Menu', 'MenuItem', 'MenuBar', 'FileUploader', 'FileDownloader', 'VideoPlayer','TreeView','TreeItem',
                      'CheckBox']
        for function_ in functions_:
            method_ = f"""def {function_}(self,  *args, **kwargs):
    if self.proxy is not None:
        kwargs['proxy'] = self.proxy
    item = {function_}(*args,**kwargs)
    self.scene.append(item)
    return item
setattr(self.__class__, '{function_}', locals()['{function_}'])"""
            exec(method_)
    ...
    #     if self.proxy is not None:
    #         kwargs['proxy'] = self.proxy
    #     fsd = FileSelectionDialog(*args,**kwargs)
