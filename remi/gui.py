...
class EventSource(object):
    ...
    def setup_event_methods(self):
        ...
        for method_name, method in inspect.getmembers(
            self, predicate=a_method_not_builtin
        ):
...
class ClassEventConnector(object):
    ...
    def do(self, callback, *userdata, **kwuserdata):
        ...
        if hasattr(self.event_method_bound, "_js_code"):
            ...
            self.event_source_instance.attributes[self.event_name] = (
                self.event_method_bound._js_code
                % {
                    "emitter_identifier": self.event_source_instance.identifier,
...
class Tag(object):
    ...
    def __init__(self, attributes=None, _type="", _class=None, **kwargs):
        ...
        self.attr_class = self.__class__.__name__ if _class is None else _class
    ...
    def _need_update(self, emitter=None, child_ignore_update=False):
        ...
        if emitter is not None:
            ...
            if len(self.style):
                ...
                tmp.pop("style", None)
            self._repr_attributes = " ".join(
                '%s="%s"' % (k, v) if v is not None else k for k, v in tmp.items()
            )
        if self.refresh_enabled:
            if self.get_parent():
                self.get_parent()._need_update(
                    child_ignore_update=(self.ignore_update or child_ignore_update)
                )
    ...
    def remove_class(self, cls):
        ...
        if len(classes) > 0:
            self.attributes["class"] = (
                " ".join(classes) if len(classes) > 1 else classes[0]
            )
        else:
            self.attributes["class"] = ""
...
class Widget(Tag, EventSource):
    ...
    @editor_attribute_decorator(
        "Generic", """The variable name used by the editor""", str, {}
    )
    ...
    @property
    @editor_attribute_decorator(
        "Generic", """Defines if to overload the base class""", bool, {}
    )
    ...
    @property
    @editor_attribute_decorator(
        "Generic", """Advisory information for the element""", str, {}
    )
    def attr_title(self):
    ...
    @property
    @editor_attribute_decorator(
        "Background", """Background color of the widget""", "ColorPicker", {}
    )
    ...
    @property
    @editor_attribute_decorator(
        "Background", """An optional background image""", "url_editor", {}
    )
    ...
    @editor_attribute_decorator(
        "Background",
        """The position of an optional background in the form 0% 0%""",
        str,
        {},
    )
    ...
    @editor_attribute_decorator(
        "Background",
        """The repeat behaviour of an optional background image""",
        "DropDown",
        {
            "possible_values": (
                "repeat",
    ...
    @editor_attribute_decorator(
        ...
        "DropDown",
        {"possible_values": ("flex-direction", "flex-wrap", "initial", "inherit")},
    )
    ...
    @editor_attribute_decorator(
        ...
        int,
        {"possible_values": "", "min": -10000, "max": 10000, "default": 1, "step": 1},
    )
    def css_order(self):
    ...
    def get_enabled(self):
        """Returns a bool."""
        return "disabled" not in self.attributes.keys()
    ...
    def query_client(self, app_instance, attribute_list, style_property_list):
        ...
        app_instance.execute_javascript(
            ...
            % {
                "attributes": ";".join(
                    map(
                        lambda param_name: "params['%(param_name)s']=document.getElementById('%(emitter_identifier)s').%(param_name)s"
                        % {
                ...
                "style": ";".join(
                    map(
                        lambda param_name: "params['%(param_name)s']=document.getElementById('%(emitter_identifier)s').style.%(param_name)s"
                        % {
                            "param_name": param_name,
                            "emitter_identifier": str(self.identifier),
                        },
...
class Container(Widget):
    ...
    def append(self, value, key=""):
        ...
        if self.layout_orientation == Container.LAYOUT_HORIZONTAL:
            if self.children[key].css_float is not None:
...
class HEAD(Tag):
    ...
    def set_icon_data(self, base64_data, mimetype="image/png", rel="icon"):
        ...
        self.add_child(
            "favicon",
            '<link rel="%s" href="%s" type="%s" />' % (rel, base64_data, mimetype),
        )
...
class BODY(Container):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loading_container = Container(
            children=[loading_anim], style={"display": "none"}
        )
...
class GridBox(Container):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Column sizes (i.e. 50% 30% 20%).""", str, {}
    )
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Row sizes (i.e. 50% 30% 20%).""", str, {}
    )
    ...
    def set_column_sizes(self, values):
        ...
        self.css_grid_template_columns = " ".join(
            map(
                lambda value: (
                    str(value) if str(value).endswith("%") else str(value) + "%"
                ),
                values,
            )
        )
    ...
    def set_from_asciiart(self, asciipattern, column_gap=0, row_gap=0):
        ...
        for c in columns.values():
            value = (
                float(c)
                / float(row_max_width)
                * (100.0 - column_gap * (len(columns) - 1))
            )
...
class HBox(Container):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.style["justify-content"] = self.style.get(
...
class AsciiContainer(Container):
    ...
    def set_from_asciiart(self, asciipattern, gap_horizontal=0, gap_vertical=0):
        ...
        for row in pattern_rows:
            ...
            for column in columns:
                ...
                if widget_key not in list(self.widget_layout_map.keys()):
                    ...
                    self.widget_layout_map[widget_key] = {
                        "width": "%.2f%%"
                        % float(widget_width / (row_width) * 100.0 - gap_horizontal),
                        "height": 1,
                        "top": "%.2f%%"
                        % float(
                            row_index / (layout_height_in_chars) * 100.0
                            + (gap_vertical / 2.0)
                        ),
                        "left": "%.2f%%"
                        % float(
                            left_value / (row_width) * 100.0 + (gap_horizontal / 2.0)
                        ),
                    }
                else:
        ...
        for key in self.widget_layout_map.keys():
            self.widget_layout_map[key]["height"] = "%.2f%%" % float(
                self.widget_layout_map[key]["height"] / (layout_height_in_chars) * 100.0
                - gap_vertical
            )
    ...
    def set_widget_layout(self, widget_key):
        if not (
            widget_key in list(self.children.keys())
            and (widget_key in list(self.widget_layout_map.keys()))
        ):
            return
...
class TabBox(Container):
    ...
    def __init__(self, *args, **kwargs):
        super(TabBox, self).__init__(
            layout_orientation=Container.LAYOUT_VERTICAL, *args, **kwargs
        )
...
class Progress(Widget):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Defines the actual value for the progress bar.""",
        int,
        {"possible_values": "", "min": 0, "max": 10000, "default": 0, "step": 1},
    )
    def attr_value(self):
...
class GenericDialog(Container):
    ...
    def __init__(self, title="", message="", *args, **kwargs):
        ...
        self.style.update(
            {"display": "block", "overflow": "auto", "margin": "0px auto"}
        )
...
class DropDown(Container):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attributes[self.EVENT_ONCHANGE] = (
            ...
            "remi.sendCallbackParam('%(id)s','%(evt)s',params);"
...
class DropDownItem(Widget, _MixinTextualWidget):
    ...
    def value(self, value):
        self.attributes["value"] = escape(value.replace("&nbsp;", " "), quote=False)
...
class Image(Widget):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Image data or url""", "base64_image", {}
    )
    def attr_src(self):
...
class SpinBox(Input):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Defines the actual value for the spin box.""",
        float,
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Defines the minimum value for the spin box.""",
        float,
        {"possible_values": "", "min": -65535, "max": 65535, "default": 0, "step": 1},
    )
    def attr_min(self):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Defines the maximum value for the spin box.""",
        float,
        {"possible_values": "", "min": -65535, "max": 65535, "default": 0, "step": 1},
    )
    def attr_max(self):
    ...
    def onchange(self, value):
        ...
        try:
            _, _, _ = (
                int(value),
                int(self.attributes["min"]),
                int(self.attributes["max"]),
            )
        except:
            _type = float
...
class SelectionInput(Input):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Defines the actual value for the widget.""", str, {}
    )
    def attr_value(self):
        return self.attributes.get("value", "0")
...
class SelectionInputWidget(Container):
    ...
    def __init__(
        ...
        self.selection_input = SelectionInput(
            default_value,
            input_type,
...
class FileFolderNavigator(GridBox):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Defines wether it is possible to select multiple items.""",
        bool,
        {},
    )
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Defines the actual navigator location.""", str, {}
    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.define_grid(
            [("button_back", "url_editor", "button_go"), ("items", "items", "items")]
        )
    ...
    def chdir(self, directory):
        ...
        for c in self.folderItems:
            self.itemContainer.remove_child(
                c
            )  # remove the file and folders from the view
        self.folderItems = []
...
class Menu(Container):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(Menu, self).__init__(
            layout_orientation=Container.LAYOUT_HORIZONTAL, *args, **kwargs
        )
        self.type = "ul"
...
class FileUploader(Container):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Defines the path where to save the file""", str, {}
    )
    def savepath(self):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attributes[self.EVENT_ONCHANGE] = (
            ...
            % {
                "id": self.identifier,
                "evt_success": self.EVENT_ON_SUCCESS,
                "evt_failed": self.EVENT_ON_FAILED,
...
class VideoPlayer(Widget):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Video poster img""", "base64_image", {}
    )
...
class _MixinSvgStroke:
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Color for svg elements.""", "ColorPicker", {}
    )
...
class _MixinTransformable:
    ...
    @editor_attribute_decorator(
        "Transformation",
        """Transform commands (i.e. rotate(45), translate(30,100)).""",
        str,
        {},
    )
...
class _MixinSvgFill:
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Fill color for svg elements.""", "ColorPicker", {}
    )
    def attr_fill(self):
...
class SvgStop(Tag):
    ...
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Gradient color""", "ColorPicker", {}
    )
    ...
    def __init__(
        self, offset="0%", color="rgb(255,255,0)", opacity=1.0, *args, **kwargs
    ):
...
class SvgGradientLinear(Tag):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Gradient coordinate value. It is expressed in percentage""",
        float,
        {"possible_values": "", "min": 0, "max": 100, "default": 0, "step": 1},
    )
    def attr_x1(self):
...
class SvgGradientRadial(Tag):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Gradient radius value. It is expressed in percentage""",
        float,
        {"possible_values": "", "min": 0, "max": 100, "default": 0, "step": 1},
    )
    def attr_r(self):
    ...
    def __init__(
        self, cx="20%", cy="30%", r="30%", fx="50%", fy="50%", *args, **kwargs
    ):
...
class Svg(Container):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """viewBox of the svg drawing. es='x, y, width, height' """,
        "str",
        {},
    )
...
class SvgRectangle(
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Horizontal round corners value.""",
        float,
        {
            "possible_values": "",
            "min": 0.0,
...
class SvgCircle(Widget, _MixinSvgStroke, _MixinSvgFill, _MixinTransformable):
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Center coordinate for SvgCircle.""",
        float,
        {
            "possible_values": "",
            "min": -65535.0,
...
class SvgText(
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Controls how text is stretched to fit the length.""",
        "DropDown",
    ...
    @editor_attribute_decorator(
        "WidgetSpecific",
        """Rotation angle for svg elements.""",
        float,
        {
            "possible_values": "",
            "min": -360.0,
...
class SvgPath(Widget, _MixinSvgStroke, _MixinSvgFill, _MixinTransformable):
    @property
    @editor_attribute_decorator(
        "WidgetSpecific", """Instructions for SvgPath.""", str, {}
    )
    def attr_d(self):
    ...
    def add_arc(self, x, y, rx, ry, x_axis_rotation, large_arc_flag, sweep_flag):
        ...
        self.attributes["d"] = (
            self.attributes["d"]
            + "A %(rx)s %(ry)s, %(x-axis-rotation)s, %(large-arc-flag)s, %(sweep-flag)s, %(x)s %(y)s"
            % {
                "x": x,
                "y": y,
                "rx": rx,
