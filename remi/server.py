...
class WebSocketsHandler(socketserver.StreamRequestHandler):
        ...
        try:
            ...
            while fin == 0:
                ...
                for char in self.rfile.read(length):
                    if is_masked:
                        next_data = chr(
                            self.bytetonum(char) ^ masks[len(frame_data) % 4]
                        )
    ...
    def send_message(self, message):
        ...
        self._log.debug(
            "send_message: %s... -> %s" % (message[:10], self.client_address)
        )
        out = bytearray()
        out.append(129)
        length = len(message)
        ...
        )  # last parameter is timeout, when 0 is non blocking
    ...
    def handshake(self):
        ...
        if self.session is None:
            return False
        if self.session not in clients.keys():
            return False
            ...
            try:
                ...
                if len(chunks) > 3:  # msgtype,widget,function,params
                    ...
                    if chunks[0] == msg_type:
                        ...
                        callback = get_method_by_name(
                            runtimeInstances[widget_id], function_name
                        )
    ...
    def close(self, terminate_server=True):
        try:
            ...
            self._log.error(
                "exception in WebSocketsHandler.close method", exc_info=True
            )
...
class App(BaseHTTPRequestHandler, object):
    ...
    re_static_file = re.compile(
        r"^([\/]*[\w\d]+:[-_. $@?#£'%=()\/\[\]!+°§^,\w\d]+)"
    )  # https://regex101.com/r/uK1sX1/6
    re_attr_call = re.compile(
        r"^/*(\w+)\/(\w+)\?{0,1}(\w*\={1}([^&])+\&{0,1})*$"
    )  # https://regex101.com/r/UTJB6N/1
    ...
    def _instance(self):
        ...
        if self.session not in clients:
            ...
            head.add_child(
                "internal_css", "<link href='/res:style.css' rel='stylesheet' />\n"
            )
    ...
    def _net_interface_ip(self):
        return self.headers.get(
            "Host",
            "%s:%s" % (self.connection.getsockname()[0], self.server.server_address[1]),
        )
            ...
            with self.update_lock:
                ...
                if self._need_update_flag:
                    try:
                        ...
                        self._log.error(
                            """exception during gui update. It is advisable to 
                            use App.update_lock using external threads.""",
                            exc_info=True,
                        )
        ...
        with self.update_lock:
            ...
            for widget in changed_widget_dict.keys():
                ...
                self._send_spontaneous_websocket_message(
                    ...
                    + __id
                    + ","
                    + to_websocket(self._overload(html, filename="internal"))
                )
        self._need_update_flag = False
    ...
    def websocket_handshake_done(self, ws_instance_to_update):
        ...
        with self.update_lock:
            msg = (
                ...
                + to_websocket(
                    self._overload(
                        self.page.children["body"].innerHTML({}), filename="internal"
                    )
                )
            )
    ...
    def _send_spontaneous_websocket_message(self, message):
        for ws in list(self.websockets):
            ...
            self._log.debug(
                "removing websocket instance, communication error with client"
            )
        ...
        try:
            ...
            for field in form.keys():
                ...
                if field_item.filename:
                    ...
                    self._log.debug(
                        'post: uploaded %s as "%s" (%d bytes)\n'
                        % (field, field_item.filename, file_len)
                    )
                    get_method_by_name(listener_widget, listener_function)(
                        file_data, filename
                    )
                else:
                    # Regular form value
            ...
            if file_data is not None:
                ...
                self._log.debug(
                    "GUI - server.py do_POST: fileupload name= %s" % (filename)
                )
    ...
    def do_GET(self):
        ...
        if "Upgrade" in self.headers:
            if self.headers["Upgrade"].lower() == "websocket":
                ...
                WebSocketsHandler(
                    self.headers, self.request, self.client_address, self.server
                )
                return
        ...
        if self.server.auth is None:
            ...
            if (
                ...
                or self.headers["Authorization"] is None
            ):
        ...
        if do_process:
            ...
            try:
                ...
                with self.update_lock:
                    ...
                    if "root" not in self.page.children["body"].children.keys():
    ...
    def all_paths(self):
        ...
        if not type(static_paths) == dict:
            self._log.error(
                "App's parameter static_file_path must be a Dictionary.", exc_info=False
            )
            static_paths = {}
    ...
    def _process_all(self, func, **kwargs):
        ...
        if (func == "/") or (not func):
            self.send_response(200)
            self.send_header(
                "Set-Cookie", "remi_session=%s; SameSite=Lax" % (self.session)
            )
            self.send_header("Content-type", "text/html")
            self.end_headers()
            ...
            self.wfile.write(
                encode_text(self._overload(page_content, filename="internal"))
            )
            ...
            self.send_response(200)
            self.send_header(
                "Content-type", mimetype if mimetype else "application/octet-stream"
            )
            ...
            with self.update_lock:
                ...
                try:
                    content, headers = get_method_by_name(
                        get_method_by_id(widget), func
                    )(**param_dict)
                    if content is None:
                    ...
                    self._log.error(
                        "attr %s/%s call error" % (widget, func), exc_info=True
                    )
                    self.send_response(404)
                    return
            ...
            try:
                ...
                self.wfile.write(
                    self._overload(encode_text(content), filename="internal")
                )
    ...
    def onerror(self, message, source, lineno, colno, error):
        ...
        self._log.debug(
            """App.onerror event occurred in webpage: 
            \nMESSAGE:%s\nSOURCE:%s\nLINENO:%s\nCOLNO:%s\ERROR:%s\n"""
    ...
    def onresize(self, emitter, width, height):
        ...
        self._log.debug(
            "App.onresize event occurred. Width:%s Height:%s" % (width, height)
        )
...
class Server(object):
    ...
    def start(self):
        ...
        self._sserver = ThreadedHTTPServer(
            (self._address, self._sport),
            self._gui,
            self._auth,
            self._multiple_instance,
            self._enable_file_cache,
            self._update_interval,
            self._websocket_timeout_timer_ms,
            self._pending_messages_queue_length,
            self._title,
            self,
            self._certfile,
            self._keyfile,
    ...
    def serve_forever(self):
        ...
        try:
            ...
            signal.signal(signal.SIGINT, sig_manager)
        except Exception:
...
class StandaloneServer(Server):
    ...
    def serve_forever(self):
        try:
            ...
            raise ImportError(
                "PyWebView is missing. Please install it by:\n    "
...
def start(main_gui_class, **kwargs):
    ...
    if standalone:
        StandaloneServer(main_gui_class, start=True, **kwargs)
    else:
